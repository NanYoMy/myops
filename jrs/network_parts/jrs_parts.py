...
class RegFusion(nn.Module):
    def __init__(self, in_channels, out_channels,reduction=4,bilinear=True):
        super(RegFusion, self).__init__()
        self.up = nn.ConvTranspose2d(in_channels , in_channels//2, kernel_size=2, stride=2)
        # self.conv = DoubleConvBlock(in_channels * 2, out_channels)
        self.conv = DoubleConvBlockV2(in_channels , out_channels)

    def forward(self, prev,i_0,i_1):
        prev=self.up(prev)
        main=torch.cat([prev,i_0,i_1],dim=1)
        return self.conv(main)


class DDFHead(nn.Module):
    def __init__(self, args, filter=[64, 32, 16, 8, 4]):
        super().__init__()
        self.args=args
        self.fusions=nn.ModuleList()
        for i in range(len(filter[:-1])):
            self.fusions.append(RegFusion(filter[i],filter[i+1]))

        self.outc=ConvBlock(filter[-1], filter[-1] // 2)

        self.flow = torch.nn.Conv2d(filter[-1]//2, 2, kernel_size=3, padding=1)

        # init flow layer with small weights and bias
        self.flow.weight = nn.Parameter(Normal(0, 1e-5).sample(self.flow.weight.shape))
        self.flow.bias = nn.Parameter(torch.zeros(self.flow.bias.shape))

    def forward(self, previous, skips_A, skips_B ):
        for i,fus in enumerate(self.fusions):
            skip_de=skips_A[-i - 1]
            skip_t2=skips_B[-i - 1]
            previous=fus(previous,skip_de,skip_t2)
        return self.flow(self.outc(previous))